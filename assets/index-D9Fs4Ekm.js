(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))x(s);new MutationObserver(s=>{for(const i of s)if(i.type==="childList")for(const u of i.addedNodes)u.tagName==="LINK"&&u.rel==="modulepreload"&&x(u)}).observe(document,{childList:!0,subtree:!0});function M(s){const i={};return s.integrity&&(i.integrity=s.integrity),s.referrerPolicy&&(i.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?i.credentials="include":s.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function x(s){if(s.ep)return;s.ep=!0;const i=M(s);fetch(s.href,i)}})();const je=`@group(0) @binding(0) var samplerLinear : sampler;\r
...
`,ir=`// normalise.wgsl  – per-channel mean-std normalisation\r
\r
struct Stats {\r
    mean : vec3<f32>,\r
...
`,L=document.getElementById("canvas"),v=document.getElementById("video"),or=document.getElementById("grad-l1"),ar=document.getElementById("video-fps"),ur=document.getElementById("inference-fps");(async()=>{const E=await navigator.gpu.requestAdapter();if(!E){alert("WebGPU not supported");return}const e=await E.requestDevice(),M=L.getContext("webgpu"),x=navigator.gpu.getPreferredCanvasFormat();M.configure({device:e,format:x});const s=await navigator.mediaDevices.getUserMedia({video:!0});v.srcObject=s,await new Promise(n=>v.onloadedmetadata=n),L.width=v.videoWidth,L.height=v.videoHeight;const i=[L.width,L.height],u=GPUTextureUsage.TEXTURE_BINDING,g=GPUTextureUsage.STORAGE_BINDING,C=GPUTextureUsage.RENDER_ATTACHMENT,b=GPUTextureUsage.COPY_DST,ve=GPUTextureUsage.COPY_SRC,c=(n,t="rgba8unorm",a=i)=>e.createTexture({size:a,format:t,usage:n}),p=e.createSampler({magFilter:"linear",minFilter:"linear"}),V=3,d=Array.from({length:V},(n,t)=>[Math.max(1,i[0]>>t),Math.max(1,i[1]>>t)]);let _=d.map(n=>c(u|C|b,"bgra8unorm",n)),m=d.map(n=>c(u|C|b,"bgra8unorm",n)),P=d.map(n=>c(u|g|b,"rgba16float",n));const y=c(u|b|C),q=c(u|g),O=c(u|g),I=c(u|g),N=c(u|g|C),K=c(u|g),Y=c(u|g),A=c(u|g|ve,"rgba8unorm",i),H=c(u|g,"rgba8unorm"),j=c(u|b,"rgba8unorm",i);let z=new Float32Array(9).fill(.1);const h=e.createTexture({size:[3,3],format:"r32float",usage:u|b});e.queue.writeTexture({texture:h},z,{bytesPerRow:3*4},[3,3]);const X=e.createShaderModule({code:je}),xe=e.createShaderModule({code:Xe}),be=e.createShaderModule({code:Je}),ye=e.createShaderModule({code:Qe}),we=e.createShaderModule({code:Ze}),_e=e.createShaderModule({code:$e}),Pe=e.createShaderModule({code:er}),J=e.createShaderModule({code:rr}),he=e.createShaderModule({code:nr}),Be=e.createShaderModule({code:tr}),Se=e.createShaderModule({code:ir}),Q=e.createRenderPipeline({layout:"auto",vertex:{module:X,entryPoint:"vs_main"},fragment:{module:X,entryPoint:"fs_main",targets:[{format:x}]},primitive:{topology:"triangle-strip"}}),Z=e.createComputePipeline({layout:"auto",compute:{module:Pe,entryPoint:"main"}}),$=e.createComputePipeline({layout:"auto",compute:{module:_e,entryPoint:"main"}}),w=e.createComputePipeline({layout:"auto",compute:{module:xe,entryPoint:"main"}}),U=e.createComputePipeline({layout:"auto",compute:{module:be,entryPoint:"main"}}),ee=e.createComputePipeline({layout:"auto",compute:{module:ye,entryPoint:"main"}}),re=e.createComputePipeline({layout:"auto",compute:{module:we,entryPoint:"main"}}),ne=e.createComputePipeline({layout:"auto",compute:{module:he,entryPoint:"main"}}),te=e.createComputePipeline({layout:"auto",compute:{module:Be,entryPoint:"main"}}),ie=e.createRenderPipeline({layout:"auto",vertex:{module:J,entryPoint:"vs_main"},fragment:{module:J,entryPoint:"fs_main",targets:[{format:x}]},primitive:{topology:"triangle-strip"}}),oe=e.createComputePipeline({layout:"auto",compute:{module:Se,entryPoint:"main"}}),B=e.createBuffer({size:9*4,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC|GPUBufferUsage.COPY_DST}),k=e.createBuffer({size:9*4,usage:GPUBufferUsage.COPY_DST|GPUBufferUsage.MAP_READ}),W=(n=32)=>e.createBuffer({size:n,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),ae=W(),ue=W(),se=W(16),S=e.createBuffer({size:8,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST,mappedAtCreation:!0});new Float32Array(S.getMappedRange()).set(i),S.unmap();const ce=e.createBuffer({size:8,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});function Ge(n,t){return e.createBindGroup({layout:Z.getBindGroupLayout(0),entries:[{binding:0,resource:n.createView()},{binding:1,resource:p},{binding:2,resource:t.createView()}]})}function Te(n,t,a){return e.createBindGroup({layout:$.getBindGroupLayout(0),entries:[{binding:0,resource:n.createView()},{binding:1,resource:t.createView()},{binding:2,resource:a.createView()}]})}function Le(n,t,a){return e.createBindGroup({layout:ne.getBindGroupLayout(0),entries:[{binding:0,resource:n.createView()},{binding:1,resource:t.createView()},{binding:2,resource:a.createView()},{binding:3,resource:p}]})}function Me(n,t){return e.createBindGroup({layout:te.getBindGroupLayout(0),entries:[{binding:0,resource:n.createView()},{binding:1,resource:t.createView()},{binding:2,resource:{buffer:B}}]})}function Ce(){return e.createBindGroup({layout:ie.getBindGroupLayout(0),entries:[{binding:0,resource:p},{binding:1,resource:y.createView()},{binding:2,resource:A.createView()}]})}const Ve=e.createBindGroup({layout:U.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:ae}},{binding:1,resource:p},{binding:2,resource:y.createView()},{binding:3,resource:O.createView()}]}),Ae=e.createBindGroup({layout:U.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:ue}},{binding:1,resource:p},{binding:2,resource:y.createView()},{binding:3,resource:I.createView()}]}),Ue=e.createBindGroup({layout:w.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:S}},{binding:1,resource:O.createView()},{binding:2,resource:h.createView()},{binding:3,resource:N.createView()}]}),ke=e.createBindGroup({layout:w.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:S}},{binding:1,resource:I.createView()},{binding:2,resource:h.createView()},{binding:3,resource:K.createView()}]}),Ee=e.createBindGroup({layout:ee.getBindGroupLayout(0),entries:[{binding:0,resource:K.createView()},{binding:1,resource:Y.createView()},{binding:2,resource:{buffer:se}},{binding:3,resource:p}]}),Oe=e.createBindGroup({layout:re.getBindGroupLayout(0),entries:[{binding:0,resource:N.createView()},{binding:1,resource:Y.createView()},{binding:2,resource:O.createView()},{binding:3,resource:{buffer:B}}]}),ze=e.createBindGroup({layout:w.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:S}},{binding:1,resource:y.createView()},{binding:2,resource:h.createView()},{binding:3,resource:A.createView()}]}),We=e.createBindGroup({layout:oe.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:ce}},{binding:1,resource:p},{binding:2,resource:y.createView()},{binding:3,resource:q.createView()}]});function de(){const n=.6+Math.random()*.3,t=Math.random()*(1-n),a=Math.random()*(1-n),r=()=>.1+Math.random()*.8;return new Float32Array([t,a,n,n,r(),r(),r(),0])}function De(){return new Float32Array([(Math.random()-.5)*.2,(Math.random()-.5)*.2,1,1])}function f(n,t,a){n.setPipeline(t),n.setBindGroup(0,a),n.dispatchWorkgroups(Math.ceil(i[0]/8),Math.ceil(i[1]/8))}function le(n,t){const a=e.createCommandEncoder(),r=a.beginRenderPass({colorAttachments:[{view:t.createView(),loadOp:"clear",storeOp:"store",clearValue:[0,0,0,1]}]}),o=e.createBindGroup({layout:Q.getBindGroupLayout(0),entries:[{binding:0,resource:p},{binding:1,resource:n.createView()}]});r.setPipeline(Q),r.setBindGroup(0,o),r.draw(4),r.end(),e.queue.submit([a.finish()])}const G=32,T=24,ge=new OffscreenCanvas(G,T).getContext("2d");function Fe(){ge.drawImage(v,0,0,G,T);const n=ge.getImageData(0,0,G,T).data;let t=0,a=0;for(let l=0;l<n.length;l+=4){const R=n[l]/255;t+=R,a+=R*R}t/=G*T;const r=a/(G*T)-t*t,o=1/Math.sqrt(r+1e-6);e.queue.writeBuffer(ce,0,new Float32Array([t,o]))}const Re=10,qe=.01,Ie=1e6;async function Ne(){const n=e.createCommandEncoder();n.copyBufferToBuffer(B,0,k,0,B.size),e.queue.submit([n.finish()]),await k.mapAsync(GPUMapMode.READ);const t=new Int32Array(k.getMappedRange());let a=0;for(let r=0;r<9;r++){const o=t[r]/Ie;a+=Math.abs(o),z[r]-=qe*o}k.unmap(),e.queue.writeTexture({texture:h},z,{bytesPerRow:3*4},[3,3]),e.queue.writeBuffer(B,0,new Float32Array(9)),or.textContent=a.toExponential(3)}let Ke=0,D=0,F=0,pe=performance.now(),me=performance.now();function Ye(){++D&&performance.now()-pe>=1e3&&(ar.textContent=String(D),D=0,pe=performance.now())}function He(){++F&&performance.now()-me>=1e3&&(ur.textContent=String(F),F=0,me=performance.now())}function fe(){e.queue.copyExternalImageToTexture({source:v},{texture:y},i),Ye(),Fe();{const r=e.createCommandEncoder(),o=r.beginComputePass();o.setPipeline(oe),o.setBindGroup(0,We),o.dispatchWorkgroups(Math.ceil(i[0]/8),Math.ceil(i[1]/8)),o.end(),e.queue.submit([r.finish()])}{const r=e.createCommandEncoder();r.copyTextureToTexture({texture:q},{texture:m[0]},i),e.queue.submit([r.finish()])}e.queue.copyExternalImageToTexture({source:v},{texture:m[0]},i);for(let r=1;r<V;r++)le(m[r-1],m[r]),le(_[r-1],_[r]);for(let r=V-1;r>=0;r--){const o=e.createCommandEncoder(),l=o.beginComputePass();r<V-1?(l.setPipeline(Z),l.setBindGroup(0,Ge(P[r+1],P[r])),l.dispatchWorkgroups(Math.ceil(d[r][0]/8),Math.ceil(d[r][1]/8))):e.queue.writeTexture({texture:P[r]},new Float32Array(d[r][0]*d[r][1]*4).fill(0),{bytesPerRow:d[r][0]*16},d[r]),l.setPipeline($),l.setBindGroup(0,Te(_[r],m[r],P[r])),l.dispatchWorkgroups(Math.ceil(d[r][0]/8),Math.ceil(d[r][1]/8)),l.end(),e.queue.submit([o.finish()])}{const r=e.createCommandEncoder(),o=r.beginComputePass();o.setPipeline(ne),o.setBindGroup(0,Le(j,P[0],H)),o.dispatchWorkgroups(Math.ceil(i[0]/8),Math.ceil(i[1]/8)),o.end(),e.queue.submit([r.finish()])}{const r=e.createCommandEncoder(),o=r.beginComputePass();o.setPipeline(te),o.setBindGroup(0,Me(A,H)),o.dispatchWorkgroups(Math.ceil(i[0]/8),Math.ceil(i[1]/8)),o.end(),e.queue.submit([r.finish()])}e.queue.writeBuffer(ae,0,de()),e.queue.writeBuffer(ue,0,de()),e.queue.writeBuffer(se,0,De());const n=e.createCommandEncoder(),t=n.beginComputePass();f(t,U,Ve),f(t,U,Ae),f(t,w,Ue),f(t,w,ke),f(t,ee,Ee),f(t,re,Oe),f(t,w,ze),t.end();const a=n.beginRenderPass({colorAttachments:[{view:M.getCurrentTexture().createView(),loadOp:"clear",storeOp:"store",clearValue:[0,0,0,1]}]});a.setPipeline(ie),a.setBindGroup(0,Ce()),a.draw(4),a.end(),e.queue.submit([n.finish()]),He();{const r=e.createCommandEncoder();r.copyTextureToTexture({texture:A},{texture:j},i),e.queue.submit([r.finish()])}++Ke%Re===0&&Ne().catch(console.error),[m,_]=[_,m],requestAnimationFrame(fe)}requestAnimationFrame(fe)})().catch(console.error);
