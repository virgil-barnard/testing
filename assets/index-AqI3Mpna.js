...
`,T=document.getElementById("canvas"),v=document.getElementById("video"),ar=document.getElementById("grad-l1"),ur=document.getElementById("video-fps"),sr=document.getElementById("inference-fps");(async()=>{const O=await navigator.gpu.requestAdapter();if(!O){alert("WebGPU not supported");return}const e=await O.requestDevice(),L=T.getContext("webgpu"),x=navigator.gpu.getPreferredCanvasFormat();L.configure({device:e,format:x});const s=await navigator.mediaDevices.getUserMedia({video:!0});v.srcObject=s,await new Promise(n=>v.onloadedmetadata=n),T.width=v.videoWidth,T.height=v.videoHeight;const o=[T.width,T.height],u=GPUTextureUsage.TEXTURE_BINDING,p=GPUTextureUsage.STORAGE_BINDING,M=GPUTextureUsage.RENDER_ATTACHMENT,b=GPUTextureUsage.COPY_DST,fe=GPUTextureUsage.COPY_SRC,d=(n,i="rgba8unorm",a=o)=>e.createTexture({size:a,format:i,usage:n}),g=e.createSampler({magFilter:"linear",minFilter:"linear"}),C=3,l=Array.from({length:C},(n,i)=>[Math.max(1,o[0]>>i),Math.max(1,o[1]>>i)]);let w=l.map(n=>d(u|M|b,"bgra8unorm",n)),m=l.map(n=>d(u|M|b,"bgra8unorm",n)),_=l.map(n=>d(u|p|b,"rgba16float",n));const P=d(u|b|M);d(u|p);const z=d(u|p),I=d(u|p),N=d(u|p|M),K=d(u|p),Y=d(u|p),V=d(u|p|fe,"rgba8unorm",o),H=d(u|p,"rgba8unorm"),j=d(u|b,"rgba8unorm",o);let W=new Float32Array(9).fill(.1);const h=e.createTexture({size:[3,3],format:"r32float",usage:u|b});e.queue.writeTexture({texture:h},W,{bytesPerRow:3*4},[3,3]);const X=e.createShaderModule({code:Xe}),ve=e.createShaderModule({code:Je}),xe=e.createShaderModule({code:Qe}),be=e.createShaderModule({code:Ze}),ye=e.createShaderModule({code:$e}),we=e.createShaderModule({code:er}),_e=e.createShaderModule({code:rr}),J=e.createShaderModule({code:nr}),Pe=e.createShaderModule({code:tr}),he=e.createShaderModule({code:ir}),Be=e.createShaderModule({code:or}),Q=e.createRenderPipeline({layout:"auto",vertex:{module:X,entryPoint:"vs_main"},fragment:{module:X,entryPoint:"fs_main",targets:[{format:x}]},primitive:{topology:"triangle-strip"}}),Z=e.createComputePipeline({layout:"auto",compute:{module:_e,entryPoint:"main"}}),$=e.createComputePipeline({layout:"auto",compute:{module:we,entryPoint:"main"}}),y=e.createComputePipeline({layout:"auto",compute:{module:ve,entryPoint:"main"}}),A=e.createComputePipeline({layout:"auto",compute:{module:xe,entryPoint:"main"}}),ee=e.createComputePipeline({layout:"auto",compute:{module:be,entryPoint:"main"}}),re=e.createComputePipeline({layout:"auto",compute:{module:ye,entryPoint:"main"}}),ne=e.createComputePipeline({layout:"auto",compute:{module:Pe,entryPoint:"main"}}),te=e.createComputePipeline({layout:"auto",compute:{module:he,entryPoint:"main"}}),ie=e.createRenderPipeline({layout:"auto",vertex:{module:J,entryPoint:"vs_main"},fragment:{module:J,entryPoint:"fs_main",targets:[{format:x}]},primitive:{topology:"triangle-strip"}}),oe=e.createComputePipeline({layout:"auto",compute:{module:Be,entryPoint:"main"}}),B=e.createBuffer({size:9*4,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC|GPUBufferUsage.COPY_DST}),U=e.createBuffer({size:9*4,usage:GPUBufferUsage.COPY_DST|GPUBufferUsage.MAP_READ}),D=(n=32)=>e.createBuffer({size:n,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),ae=D(),ue=D(),se=D(16),S=e.createBuffer({size:8,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST,mappedAtCreation:!0});new Float32Array(S.getMappedRange()).set(o),S.unmap();const Se=e.createBuffer({size:32,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});function Ge(n,i){return e.createBindGroup({layout:Z.getBindGroupLayout(0),entries:[{binding:0,resource:n.createView()},{binding:1,resource:g},{binding:2,resource:i.createView()}]})}function Te(n,i,a){return e.createBindGroup({layout:$.getBindGroupLayout(0),entries:[{binding:0,resource:n.createView()},{binding:1,resource:i.createView()},{binding:2,resource:a.createView()}]})}function Le(n,i,a){return e.createBindGroup({layout:ne.getBindGroupLayout(0),entries:[{binding:0,resource:n.createView()},{binding:1,resource:i.createView()},{binding:2,resource:a.createView()},{binding:3,resource:g}]})}function Me(n,i){return e.createBindGroup({layout:te.getBindGroupLayout(0),entries:[{binding:0,resource:n.createView()},{binding:1,resource:i.createView()},{binding:2,resource:{buffer:B}}]})}function Ce(){return e.createBindGroup({layout:ie.getBindGroupLayout(0),entries:[{binding:0,resource:g},{binding:1,resource:P.createView()},{binding:2,resource:V.createView()}]})}const Ve=e.createBindGroup({layout:A.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:ae}},{binding:1,resource:g},{binding:2,resource:P.createView()},{binding:3,resource:z.createView()}]}),Ae=e.createBindGroup({layout:A.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:ue}},{binding:1,resource:g},{binding:2,resource:P.createView()},{binding:3,resource:I.createView()}]}),Ue=e.createBindGroup({layout:y.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:S}},{binding:1,resource:z.createView()},{binding:2,resource:h.createView()},{binding:3,resource:N.createView()}]}),ke=e.createBindGroup({layout:y.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:S}},{binding:1,resource:I.createView()},{binding:2,resource:h.createView()},{binding:3,resource:K.createView()}]}),Ee=e.createBindGroup({layout:ee.getBindGroupLayout(0),entries:[{binding:0,resource:K.createView()},{binding:1,resource:Y.createView()},{binding:2,resource:{buffer:se}},{binding:3,resource:g}]}),Oe=e.createBindGroup({layout:re.getBindGroupLayout(0),entries:[{binding:0,resource:N.createView()},{binding:1,resource:Y.createView()},{binding:2,resource:z.createView()},{binding:3,resource:{buffer:B}}]}),ze=e.createBindGroup({layout:y.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:S}},{binding:1,resource:P.createView()},{binding:2,resource:h.createView()},{binding:3,resource:V.createView()}]}),We=e.createBindGroup({layout:oe.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:statsBuf}},{binding:1,resource:m[0].createView()}]});function ce(){const n=.6+Math.random()*.3,i=Math.random()*(1-n),a=Math.random()*(1-n),r=()=>.1+Math.random()*.8;return new Float32Array([i,a,n,n,r(),r(),r(),0])}function De(){return new Float32Array([(Math.random()-.5)*.2,(Math.random()-.5)*.2,1,1])}function f(n,i,a){n.setPipeline(i),n.setBindGroup(0,a),n.dispatchWorkgroups(Math.ceil(o[0]/8),Math.ceil(o[1]/8))}function de(n,i){const a=e.createCommandEncoder(),r=a.beginRenderPass({colorAttachments:[{view:i.createView(),loadOp:"clear",storeOp:"store",clearValue:[0,0,0,1]}]}),t=e.createBindGroup({layout:Q.getBindGroupLayout(0),entries:[{binding:0,resource:g},{binding:1,resource:n.createView()}]});r.setPipeline(Q),r.setBindGroup(0,t),r.draw(4),r.end(),e.queue.submit([a.finish()])}const k=32,E=24,le=new OffscreenCanvas(k,E).getContext("2d"),F=new Float32Array(8);function Fe(){le.drawImage(v,0,0,k,E);const n=le.getImageData(0,0,k,E).data,i=[0,0,0],a=[0,0,0],r=k*E;for(let t=0;t<n.length;t+=4)for(let c=0;c<3;c++){const G=n[t+c]/255;i[c]+=G,a[c]+=G*G}for(let t=0;t<3;t++){const c=i[t]/r,G=a[t]/r-c*c,je=1/Math.sqrt(G+1e-6);F[t]=c,F[4+t]=je}e.queue.writeBuffer(Se,0,F)}const Re=10,qe=.01,Ie=1e6;async function Ne(){const n=e.createCommandEncoder();n.copyBufferToBuffer(B,0,U,0,B.size),e.queue.submit([n.finish()]),await U.mapAsync(GPUMapMode.READ);const i=new Int32Array(U.getMappedRange());let a=0;for(let r=0;r<9;r++){const t=i[r]/Ie;a+=Math.abs(t),W[r]-=qe*t}U.unmap(),e.queue.writeTexture({texture:h},W,{bytesPerRow:3*4},[3,3]),e.queue.writeBuffer(B,0,new Float32Array(9)),ar.textContent=a.toExponential(3)}let Ke=0,R=0,q=0,pe=performance.now(),ge=performance.now();function Ye(){++R&&performance.now()-pe>=1e3&&(ur.textContent=String(R),R=0,pe=performance.now())}function He(){++q&&performance.now()-ge>=1e3&&(sr.textContent=String(q),q=0,ge=performance.now())}function me(){e.queue.copyExternalImageToTexture({source:v},{texture:P},o),Ye(),Fe();{const r=e.createCommandEncoder(),t=r.beginComputePass();t.setPipeline(oe),t.setBindGroup(0,We),t.dispatchWorkgroups(Math.ceil(o[0]/8),Math.ceil(o[1]/8)),t.end(),e.queue.submit([r.finish()])}e.queue.copyExternalImageToTexture({source:v},{texture:m[0]},o);for(let r=1;r<C;r++)de(m[r-1],m[r]),de(w[r-1],w[r]);for(let r=C-1;r>=0;r--){const t=e.createCommandEncoder(),c=t.beginComputePass();r<C-1?(c.setPipeline(Z),c.setBindGroup(0,Ge(_[r+1],_[r])),c.dispatchWorkgroups(Math.ceil(l[r][0]/8),Math.ceil(l[r][1]/8))):e.queue.writeTexture({texture:_[r]},new Float32Array(l[r][0]*l[r][1]*4).fill(0),{bytesPerRow:l[r][0]*16},l[r]),c.setPipeline($),c.setBindGroup(0,Te(w[r],m[r],_[r])),c.dispatchWorkgroups(Math.ceil(l[r][0]/8),Math.ceil(l[r][1]/8)),c.end(),e.queue.submit([t.finish()])}{const r=e.createCommandEncoder(),t=r.beginComputePass();t.setPipeline(ne),t.setBindGroup(0,Le(j,_[0],H)),t.dispatchWorkgroups(Math.ceil(o[0]/8),Math.ceil(o[1]/8)),t.end(),e.queue.submit([r.finish()])}{const r=e.createCommandEncoder(),t=r.beginComputePass();t.setPipeline(te),t.setBindGroup(0,Me(V,H)),t.dispatchWorkgroups(Math.ceil(o[0]/8),Math.ceil(o[1]/8)),t.end(),e.queue.submit([r.finish()])}e.queue.writeBuffer(ae,0,ce()),e.queue.writeBuffer(ue,0,ce()),e.queue.writeBuffer(se,0,De());const n=e.createCommandEncoder(),i=n.beginComputePass();f(i,A,Ve),f(i,A,Ae),f(i,y,Ue),f(i,y,ke),f(i,ee,Ee),f(i,re,Oe),f(i,y,ze),i.end();const a=n.beginRenderPass({colorAttachments:[{view:L.getCurrentTexture().createView(),loadOp:"clear",storeOp:"store",clearValue:[0,0,0,1]}]});a.setPipeline(ie),a.setBindGroup(0,Ce()),a.draw(4),a.end(),e.queue.submit([n.finish()]),He();{const r=e.createCommandEncoder();r.copyTextureToTexture({texture:V},{texture:j},o),e.queue.submit([r.finish()])}++Ke%Re===0&&Ne().catch(console.error),[m,w]=[w,m],requestAnimationFrame(me)}requestAnimationFrame(me)})().catch(console.error);
